...
function normalizeRankings(rankingsBySort) {
  ...
  return [...byHref.values()].map((item) => {
    const appearances = Object.keys(item.ranks).length;
...
- Dataset date: April 20, 2026
- Source: public ClawHub plugin ranking pages plus plugin detail SSR payloads

## Executive Summary

The ClawHub plugin market is still early. On April 20, 2026, the public plugin detail payloads expose almost no meaningful 'downloads', 'installs', or 'stars', so plugin breakout analysis has to rely on ranking position, verification quality, plugin structure, scan coherence, and author production systems rather than consumer popularity numbers alone.

## What Actually Wins

1. Code plugins outrank bundle-only packaging.
2. Strong plugin names describe the system or operational job directly.
3. Source-linked verification is a major trust accelerator.
4. The best authors ship adjacent plugins in one clear lane instead of random one-offs.
5. Security, governance, growth ops, payments, storage, and workflow connectors are the best AISA-conversion lanes.
...
async function main() {
  ...
  const report = {
    ...
    methodology: {
      note: 'ClawHub plugin detail SSR payloads currently expose public stats as zero for most plugins, so the report weights page-order ranks, verification quality, scan status, category fit, and author production density.',
      dataDate: '2026-04-20',
    ...
    rankings: {
      downloads: rankingsBySort.downloads.map((item, index) => ({ rank: index + 1, href: item.href, name: item.displayName, owner: item.owner })),
      stars: rankingsBySort.stars.map((item, index) => ({ rank: index + 1, href: item.href, name: item.displayName, owner: item.owner })),
      installs: rankingsBySort.installs.map((item, index) => ({ rank: index + 1, href: item.href, name: item.displayName, owner: item.owner })),
  ...
  const zhReport = buildZhReport(report);
  const enReport = buildEnReport(report);
